...
function ENT:Initialize()
    if SERVER then
        self:SetModel(config999.water_model)
        ...
        self.ShrinkAmount = config999.growamount
        self.Cooldown = config999.water_cooldown
    end
end
...
function ENT:Use(activator, caller)
    ...
    local newScale = currentScale - (self.ShrinkAmount or 0.1)
    if newScale < self.MinScale then return end
    ...
    guthscp.player_message(activator, config999.translation_3)
    activator:EmitSound("items/battery_pickup.wav", 75, 100)
end

if CLIENT then
    list.Set("SpawnableEntities", "revscp_water_bowl", {
        PrintName = "Water Bowl",
        ClassName = "revscp_water_bowl",
        Category = "GuthSCP"
    })
end